...
object UserCredentials {
  implicit val decoder: JsonDecoder[UserCredentials] =
    DeriveJsonDecoder.gen[UserCredentials]
}
  ...
  val app: HttpApp[Users, Response] =
    Http.collectZIO[Request] {
      case req @ Method.POST -> !! / "auth" / "login" =>
        {
          (for {
            user_cred <- req.body.asString
              .map(body => body.fromJson[UserCredentials])
              .right
            password_hash = PasswordEncode.encode(user_cred.password)
            user = User(user_cred.login, password_hash)
            _ <- Users.login(user)
          } yield (Response.json(
            AuthorizationToken(JwtUtils.generateJwt(user)).toJson
          )))
